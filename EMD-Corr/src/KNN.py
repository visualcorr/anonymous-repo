...
imagenet_train_data = ImageFolder(
    ...
    root="/home/user1/Downloads/train/", transform=Dataset.imagenet_transform
)
...
for val_dataset in [val_datasets]:
    if val_dataset == Dataset.CUB200:
        imagenet_train_data = ImageFolder(
            # CUB train folder
            root="/home/user1/Downloads/cub200/train/",
    ...
    if val_dataset in Dataset.IMAGENET_C_NOISE:
        imagenet_val_data = ImageFolder(
            root="/home/user1/Downloads/shared_datasets/imagenet-c/{}/".format(
                val_dataset
            ),
            transform=Dataset.imagenet_transform,
        )
        ...
        if (
            ...
            imagenet_val_data = ImageFolder(
                root="/home/user1/Downloads/{}/".format(val_dataset),
                transform=Dataset.imagenet_transform_crop_patch,
            )
        else:
            if val_dataset == Dataset.CUB200:
                imagenet_val_data = ImageFolder(
                    root="/home/user1/Downloads/cub200/{}/".format(val_dataset),
                    transform=Dataset.imagenet_transform,
                )
            else:
                if val_dataset in [Dataset.IMAGENET_1K_50K_CLEAN, Dataset.OBJECTNET_5K, Dataset.IMAGENET_A]:
                    imagenet_val_data = ImageFolder(
                        root="/home/user1/Downloads/shared_datasets/{}/".format(val_dataset),
                        transform=Dataset.imagenet_transform,
                    )
                else:
                    imagenet_val_data = ImageFolder(
